...
public class PLatinImpl implements PLatin {
  ...
  @Override public String convert(String word) {
    ...
    Optional<Integer> vowelIsFirst = vowels.stream().map(vowel -> word.indexOf(vowel)).filter(x -> x == 0)
        .findFirst();

    Optional<Integer> minVowel = vowels.stream().map(vowel -> word.indexOf(vowel)).filter(x-> x>0 ).min(Integer::compare);
  ...
  @Override public String process(String string) {
    ...
    return Arrays.asList(strings).stream().map(word -> convert(word)).collect(Collectors.joining(" "));
